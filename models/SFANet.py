...
class SFANet(nn.Module):
    ...
    def forward(self, x):
        ...
        sfa4 = self.sfa4(feat32,gap)
        sfa3 = self.sfa3(dsn,sfa4)
        sfa2 = self.sfa2(feat8,sfa3)
        sfa1 = self.sfa1(feat4,sfa2)

        f = torch.cat([feat4,sfa1],dim=1)
        f = self.conv_out(f)
